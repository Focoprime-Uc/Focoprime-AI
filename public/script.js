...
if (activeChatId) {
  ...
  const chat = histories.find(c => c.id == activeChatId);

  if (chat) {
    chatHistory.length = 0;
    chat.messages.forEach(m => chatHistory.push(m));
...
async function saveConversationHistory(chatHistory) {
        ...
        messages: [
          {
            role: "system",
            content: `
Cria um título MUITO curto (máx. 4 palavras) que resuma a conversa.
Não uses emojis.
Não uses pontuação.
Apenas o título.
            `
          },
          ...messages.slice(0, 4) // só o início da conversa
        ]
      })
    });
